    ...
    constructor(model, metadata, geometry, material, position, rotation, scale) {
        ...
        if (geometry.isGroup) {
            ...
            for ( let i = 0, l = children.length; i < l; i ++ ) {
                let boundingBoxTemp = new  THREE.Box3();
                boundingBoxTemp.setFromObject(children[ i ]);
                let d =  boundingBoxTemp.max.y - boundingBoxTemp.min.y;
                console.log(i + " " + children[i].name + " " + d);
            }
        ...
        if (!geometry.isGroup) {
            ...
            this.traverse( function(child) {
                if (child instanceof THREE.Mesh) {
                  ...
                  //if (child.geometry.removeAttribute) {
                  //  child.geometry.removeAttribute("normal");
                  //}
                  //child.material = new THREE.MeshPhongMaterial({color: 0x55B663});
                  //child.material = new THREE.MeshPhongMaterial({color: 0x55B663,  shading: THREE.FlatShading});
        ...
        //this.helper = new THREE.BoxHelper( this, 0x0000ff );
...
//ItemGroup.prototype = Object.create(THREE.Group.prototype);
