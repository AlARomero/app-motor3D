...
// x and y are floats
var Corner = function(floorplan, x, y, id, merge, tolerance) {

  this.id = id || utils.guid();
  this.mergeCorners = merge || 1;
  ...
  this.distanceFrom = function(x, y) {
    ...
    // console.log('x,y ' + x + ',' + y + ' to ' + this.getX() + ',' +
	// this.getY() + ' is ' + distance);
    return distance;
  }
  ...
  this.mergeWithIntersected = function() {
    ...
    // check corners
    //console.log("[mergeWithIntersected] tolerance: " + tolerance);
    for( var i = 0; i < floorplan.getCorners().length; i++ ) {
      let obj = floorplan.getCorners()[i];
