...
var ThreeControls = function (object, domElement) {
	...
	this.minDistance = 0;
	this.maxDistance = 10000; // Infinity;
  ...
  this.panTo = function(vec3) {
	...
	this.pan = function ( delta ) {
		...
		if ( scope.object.fov !== undefined ) {
			...
			// we actually don't use screenWidth, since perspective camera is
	...
	this.dollyIn = function ( dollyScale ) {
		if ( dollyScale === undefined ) {
			dollyScale = getZoomScale();
                ...
                //console.log("DollyIn dollyScale: " + dollyScale);
		scale /= dollyScale;
                //console.log("DollyIn scale: " + scale);
	};
	...
	this.dollyOut = function ( dollyScale ) {
		if ( dollyScale === undefined ) {
			dollyScale = getZoomScale();
                ...
                //console.log("DollyOut dollyScale: " + dollyScale);
		scale *= dollyScale;
                //console.log("DollyOut scale: " + scale);
	};
	...
	function onMouseDown( event ) {
                ...
                utils.writeDebug("Estoy en el mousedown (THREEControls)");
		...
		if ( event.button === 0 ) {
			...
			state = STATE.ROTATE;
                        utils.writeDebug("[onMouseDown (THREEControls)] State.ROTATE");
			...
			state = STATE.DOLLY;
                        utils.writeDebug("[onMouseDown (THREEControls)] State.DOLLY");
			...
			state = STATE.PAN;
                        utils.writeDebug("[onMouseDown (THREEControls)] State.PAN");
			panStart.set( event.clientX, event.clientY );
		}

		// Greggman fix:
		// https://github.com/greggman/three.js/commit/fde9f9917d6d8381f06bf22cdff766029d1761be
	...
	function onMouseMove( event ) {
            console.debug("Estoy en el mousemove (THREEControls)");
//console.log("Estoy en el mousemove");
		if ( scope.enabled === false ) return;
	...
	function touchstart( event ) {
            console.log("Estoy en el touchdstart");
		if ( scope.enabled === false ) { return; }
		...
		switch ( event.touches.length ) {
			...
			default:
                ...
                scope.domElement.addEventListener( 'touchmove', touchmove, 
	...
	function touchmove( event ) {
            console.log("Estoy en el touchmove");
		if ( scope.enabled === false ) { return; }
		...
		var element = scope.domElement === document ? scope.domElement.body : scope.domElement;
                ...
                console.log("Dedos activos: " + event.touches.length);
		switch ( event.touches.length ) {
			...
			case 1: // one-fingered touch: rotate
				...
				if ( state !== STATE.TOUCH_ROTATE ) { return; }
                                console.log("Aqui: " + event.touches[ 0 ].pageX + " " + event.touches[ 0 ].pageY);   
			...
			case 2: // two-fingered touch: dolly
				...
				dollyEnd.set( 0, distance );
                                console.log("DollyEnd distance: " + distance);
				dollyDelta.subVectors( dollyEnd, dollyStart );
	...
	function touchend( /* event */ ) {
            console.log("Estoy en el touchend");
